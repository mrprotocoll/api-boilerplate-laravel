...
final class AIController
{
    ...
    public function createSession(CreateAISessionRequest $request): JsonResponse
    {
        try {
            ...
            $session = $this->conversationService->createOrResumeSession($request->validated(), $user);

            return ResponseHelper::success([
                'session' => AISessionResource::make($session),
                ...$this->conversationService->capabilities(),
            ], 'AI session ready');
    ...
    public function latestSession(): JsonResponse
    {
        try {
            $user = $this->authenticatedUser();
            $session = $this->conversationService->getLatestSession($user);
    ...
    {
        try {
            $response = $this->conversationService->sendMessage($request->validated(), $this->authenticatedUser());
    ...
    public function streamMessage(StoreAIMessageRequest $request): JsonResponse|StreamedResponse
    {
        ...
        return new StreamedResponse(function () use ($data, $user, $service): void {
            ...
            try {
                $response = $service->sendMessage(
                    ...
                    onUserMessage: static fn ($message): null => $emit('user_message', ['message' => AIMessageResource::make($message)->resolve()]),
    ...
    {
        try {
            $message = $this->conversationService->flagMessage($messageId, $request->validated(), $this->authenticatedUser());
